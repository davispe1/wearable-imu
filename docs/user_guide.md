...
# Desktop app (recorded-file viewer): import CSV -> pipeline -> 2D gait curves (see §7)
python -m app.main

# Export a processed session for the external Open3D viewer (see §7.5; Open3D NOT required)
...
runs the **unchanged** kincore pipeline, and visualises the result as **synchronised 2D
joint-angle and angular-velocity curves**. `run.py`/`visualize.py` remain the
...
```powershell
python -m app.main          # launch the app (needs a display + pyqtgraph, PyQt5)
```
...
- **Visualize** — the main screen, **2D only (no figure pane)**: synchronised
  joint-flexion and angular-velocity curves with a sweeping cursor (turnarounds shaded),
  and a **live parameter box** showing only the in-capture subset — current joint angle,
  accumulated ROM + peak range, angular velocity (current and peak), step count and
  cadence — plus transport controls (play/pause, speed 0.25–2×, loop), a live time
  read-out and a **timeline scrubber**. Scrubber and cursor move together.
...
The plotted curves and live parameters are reported in a **neutral-referenced** flexion
frame (per-joint mounting offset removed so a standing pose reads ~0°). Rendering is plain
**2D pyqtgraph** (`PlotWidget`) — no 3D dependency. A 3D/animated figure view is out of
scope (a possible future upgrade).
...
<data_dir>/<session_id>/        e.g. P01_S01_2minWalk_right/
   ...
   results/
      ...
      gait_frames.csv, meta.json         (written by Results → Export for Open3D; see §7.5)
```
...
### 7.5 Export for the external Open3D viewer

A processed session can be exported for a **separate Open3D viewer project**. **Open3D is not
a dependency of this repo** — this is a standalone export step (`pipeline/export_open3d.py`)
that only *reads* the gaitlib results already computed and writes two small files into the
session's `results/` folder:

| file | contents |
|---|---|
| `gait_frames.csv` | columns `t_s, hip_deg, knee_deg, ankle_deg` — one row per sample. The clean **sagittal flexion** angles straight from gaitlib (`results.joints[<joint>]["flexion"]`), **not** the 3D orientation, for the **single leg** declared in the mounting config. |
| `meta.json` | `{"leg": "<right\|left>", "fs": <Hz>, "segment_lengths_m": {"pelvis","thigh","shank","foot"}}`. The segment lengths are estimated from the subject's stature using **Winter's body-segment proportions**; when no stature is available a default (1.70 m) is used and noted in `segment_lengths_source`. |

Two equivalent ways to produce them:

```powershell
python -m pipeline.export_open3d <session_dir>     # CLI (a session folder)
```
or, in the app, **Results → "Export for Open3D"** (exports the currently loaded session).
The CLI reuses the session's cached `results/` when present, otherwise it runs gaitlib first.

**Future / live hardware.** The `gait_frames` schema *is* the viewer's frame contract: the
same per-frame record (`t_s, hip_deg, knee_deg, ankle_deg`) can later be **streamed over a
socket** from live hardware instead of written to a CSV — the per-frame schema is identical,
so the Open3D viewer needs no change to switch from file playback to a live feed.
